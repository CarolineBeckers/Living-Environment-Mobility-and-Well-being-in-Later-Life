...
#median birth year
median_birth_year <- median(data$Birth_year, na.rm = TRUE)

#1953 --> 71 years old

#select younger than median respondents

library(dplyr)

selected_rows <- data[data$Birth_year > 1953, ]
...
df <- data.frame(Comments = selected_rows$Changes)
